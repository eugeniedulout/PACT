...
Il s'agit d'une application de création de liste de courses relié à une base de donnée des produits disponibles en magasin, associé à une méthode de localisation à l'intérieur du magasin. Lors de la création de la liste, l'application propose des produits en fonction des préférences de l'utilisateur et de ses listes précédentes, mais aussides promotions actuelles dans le magasin. 
Les listes de courses peuvent être partagés entre utilisateurs, ainsi que des sous-listes (ou "recettes") de type d'ingrédients. Une fois la liste de courses rentrés, l'application fournit une fois dans le magasin la localisation de l'usager et des produits sur la listes, ainsi qu'un chemin optimal pour aller les récupérés. 
