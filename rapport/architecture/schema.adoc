...
Le serveur devra aussi effectuer des calculs en amont tels que la recherche du produit correspondant aux critères de l'utilisateur afin de permettre des résultats cohérents entre les différentes interfaces (cf ci-après). Dès lors, il est primordial d'optimiser ses calculs afin d'éviter une surcharge des requêtes. Si nous constatons que le serveur est trop vite saturé, nous envisagerons d'effectuer les calculs en local.
...
Il faudra développer une interface Web qui permettrait de gérer uniquement les listes de courses. En effet, il est plus agréable de rentrer ses produits depuis son ordinateur que depuis un smartphone ou une tablette. Cet outil n'implémentera pas la fonctionnalité de guidage dans le magasin, qui ne peut se faire sur ordinateur. Ce découpage impose donc d'effectuer le plus de calculs possibles sur le serveur afin d'éviter la redondance, et des résultats potentiellement différents entre l'application web et android.
